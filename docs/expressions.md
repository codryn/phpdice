...
```XdY advantage```, ```XdY disadvantage```
Examples:
...
Use keep highest/lowest mechanics:
 ```XdY keep N highest```, ```XdY keep N lowest```
Examples:
...
```XdY reroll <= N```, ```XdY reroll < N```, ```XdY reroll >= N```, ```XdY reroll N```
...
```XdY reroll M <= N```, ```XdY reroll M < N```, ```XdY reroll M >= N```, ```XdY reroll M N```
...
```XdY explode```, ```XdY explode >= N```, ```XdY explode M```, ```XdY explode M >= N```
Examples:
...
```$name$```
Examples:
```1d20 + $str$ + $proficiency$``` - Roll one d20, add strength and proficiency modifiers from variables
...
```XdY + Z dc >= N```, ```XdY + Z dc > N```, ```XdY + Z dc <= N```, ```XdY + Z dc < N```, ```XdY + Z dc == N```
Examples:
...
```XdY crit N```, ```XdY glitch N```, ```XdY crit N glitch M```
Examples:
...
Examples:
```1d20 advantage + 5 >= 15 crit 20``` - Roll d20 with advantage, add 5, check against DC 15, critical on 20
```12d6 reroll <=1 >=5``` - Roll 12d6, reroll any 1s, count successes of 5 or higher
